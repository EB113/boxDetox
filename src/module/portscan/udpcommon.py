...
class Module_SCAN_UDPCommon(PortScanner):
	...
	def validate(opt_dict=None):
		valid = True
		opt = dict(Module_SCAN_UDPCommon.opt_static)
		if opt_dict != None and len(opt_dict.keys()) >= len(Module_UDP_UDPCommon.opt_static.keys()):
			for k,v in opt_dict.items():
				if k in Module_SCAN_TCPCommon.opt_static:
					valid = valid and Module_SCAN_UDPCommon.opt_static.get(k,None)(v)
					try:
						opt.pop(k, None)
					except:
						print("{}".format(e))
					...
					valid = valid and Module_SCAN_UDPCommon.opt_dynamic.get(k,None)(v)
			...
			for ip in lst:
				try:
					if not os.path.isdir(fn+ip):
						os.mkdir(fn+ip)
					os.system("nmap --top-ports 1000 -sU -sV -T4 "+ip+" -oA "+fn+ip+"/udp_"+ip+" 1>/dev/null 2>/dev/null")
