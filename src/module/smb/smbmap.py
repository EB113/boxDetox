...
class Module_SMB_smbmap(Module):
	...
	def validate(opt_dict=None):
		valid = True
		opt = dict(Module_SMB_smbmap.opt_static)
		if opt_dict != None and len(opt_dict.keys()) >= len(Module_SMB_smbmap.opt_static.keys()):
			for k,v in opt_dict.items():
				if k in Module_SMB_smbmap.opt_static:
					valid = valid and Module_SMB_smbmap.opt_static.get(k,None)(v)
					try:
						opt.pop(k, None)
					except:
						print("{}".format(e))
						...
						return False
				elif k in Module_SMB_smbmap.opt_dynamic:
					valid = valid and Module_SMB_smbmap.opt_dynamic.get(k,None)(v)
	...
	def run(self):
		lst = Module_SMB_smbmap.targets(self.opt_dict["target"])
		data = {}
		for ip in lst:
			if not self.flag.is_set():
				proc = os.popen("/bin/bash -c 'smbmap -H "+ip+"'")
				out = proc.read()
				proc.close()
				if self.mode == "profile": 
					fd = open(Config.CONFIG['GENERAL']['PATH'] + "/db/sessions/" + Config.CONFIG['GENERAL']['SESSID']+"/profile/"+self.profile_tag+"/"+ip+"/"+self.profile_port+"/smbmap","w")
					fd.write(out)
					fd.close()
				data[ip] = out
