...
class Module_SMB_nmapenum(Module):
	...
	def validate(opt_dict=None):
		valid = True
		opt = dict(Module_SMB_nmapenum.opt_static)
		if opt_dict != None and len(opt_dict.keys()) >= len(Module_SMB_nmapenum.opt_static.keys()):
			for k,v in opt_dict.items():
				if k in Module_SMB_nmapenum.opt_static:
					valid = valid and Module_SMB_nmapenum.opt_static.get(k,None)(v)
					try:
						opt.pop(k, None)
					except:
						print("{}".format(e))
						...
						return False
				elif k in Module_SMB_nmapenum.opt_dynamic:
					valid = valid and Module_SMB_nmapenum.opt_dynamic.get(k,None)(v)
	...
	def run(self):
		lst = Module_SMB_nmapenum.targets(self.opt_dict["target"])
		data = {}
		for ip in lst:
			if not self.flag.is_set():
				proc = os.popen("/bin/bash -c 'nmap -p "+ self.opt_dict["port"] +" --script=smb-enum* "+ip+"'")
				out = proc.read()
				proc.close()
