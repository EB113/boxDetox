...
class Module_SMTP_nmapvuln(Module):
	...
	def validate(opt_dict):
		valid = True
		if len(opt_dict.keys()) == len(Module_SMTP_nmapvuln.opt.keys()):
			for k,v in opt_dict.items():
				valid = valid and Module_SMTP_nmapvuln.opt.get(k,None)(v)
		else:
			valid = False
		return valid
	...
	def validate(opt_dict=None):
		valid = True
		opt = dict(Module_SMTP_nmapvuln.opt_static)
		if opt_dict != None and len(opt_dict.keys()) >= len(Module_SMTP_nmapvuln.opt_static.keys()):
			for k,v in opt_dict.items():
				if k in Module_SMTP_nmapvuln.opt_static:
					valid = valid and Module_SMTP_nmapvuln.opt_static.get(k,None)(v)
					try:
						opt.pop(k, None)
					except:
						print("{}".format(e))
						...
						return False
				elif k in Module_SMTP_nmapvuln.opt_dynamic:
					valid = valid and Module_SMTP_nmapvuln.opt_dynamic.get(k,None)(v)
	...
	def run(self):
		lst = Module_SMTP_nmapvuln.targets(self.opt_dict["target"])
		data = {}
		for ip in lst:
			if not self.flag.is_set():
				proc = os.popen("/bin/bash -c 'nmap -p "+ self.opt_dict["port"] +" --script=smtp-vuln* "+ip+"'")
				out = proc.read()
				proc.close()
				if self.mode == "profile": 
					fd = open(Config.CONFIG['GENERAL']['PATH'] + "/db/sessions/" + Config.CONFIG['GENERAL']['SESSID']+"/profile/"+self.profile_tag+"/"+ip+"/"+self.profile_port+"/nmap_vuln","w")
					fd.write(out)
					fd.close()
				data[ip] = out
