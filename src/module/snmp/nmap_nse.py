...
class Module_SNMP_nmapnse(Module):
	...
	def validate(opt_dict=None):
		valid = True
		opt = dict(Module_SNMP_nmapnse.opt_static)
		if opt_dict != None and len(opt_dict.keys()) >= len(Module_SNMP_nmapnse.opt_static.keys()):
			for k,v in opt_dict.items():
				if k in Module_SNMP_nmapnse.opt_static:
					valid = valid and Module_SNMP_nmapnse.opt_static.get(k,None)(v)
					try:
						opt.pop(k, None)
					except:
						print("{}".format(e))
						...
						return False
				elif k in Module_SNMP_nmapnse.opt_dynamic:
					valid = valid and Module_SNMP_nmapnse.opt_dynamic.get(k,None)(v)
	...
	def run(self):
		lst = Module_SNMP_nmapnse.targets(self.opt_dict["target"])
		data = {}
		for ip in lst:
			if not self.flag.is_set():
				proc = os.popen("/bin/bash -c 'nmap -p "+ self.opt_dict["port"] +" --script=snmp* "+ip+"'")
				out = proc.read()
				proc.close()
				if self.mode == "profile": 
					fd = open(Config.CONFIG['GENERAL']['PATH'] + "/db/sessions/" + Config.CONFIG['GENERAL']['SESSID']+"/profile/"+self.profile_tag+"/"+ip+"/"+self.profile_port+"/nmap_nse","w")
					fd.write(out)
					fd.close()
				data[ip] = out
