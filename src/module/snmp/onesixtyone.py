...
class Module_SNMP_onesixtyone(Module):
	...
	def validate(opt_dict=None):
		valid = True
		opt = dict(Module_SNMP_onesixtyone.opt_static)
		if opt_dict != None and len(opt_dict.keys()) >= len(Module_SNMP_onesixtyone.opt_static.keys()):
			for k,v in opt_dict.items():
				if k in Module_SNMP_onesixtyone.opt_static:
					valid = valid and Module_SNMP_onesixtyone.opt_static.get(k,None)(v)
					try:
						opt.pop(k, None)
					except:
						print("{}".format(e))
						...
						return False
				elif k in Module_SNMP_onesixtyone.opt_dynamic:
					valid = valid and Module_SNMP_onesixtyone.opt_dynamic.get(k,None)(v)
