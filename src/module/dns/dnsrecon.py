...
class Module_DNS_dnsrecon(Module):
	...
	def validate(opt_dict=None):
		valid = True
		opt = dict(Module_DNS_dnsrecon.opt_static)
		if opt_dict != None and len(opt_dict.keys()) >= len(Module_DNS_dnsrecon.opt_static.keys()):
			for k,v in opt_dict.items():
				if k in Module_DNS_dnsrecon.opt_static:
					valid = valid and Module_DNS_dnsrecon.opt_static.get(k,None)(v)
					try:
						opt.pop(k, None)
					except:
						print("{}".format(e))
						...
						return False
				elif k in Module_DNS_dnsrecon.opt_dynamic:
					valid = valid and Module_DNS_dnsrecon.opt_dynamic.get(k,None)(v)
