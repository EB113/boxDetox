...
class Module_HTTPnikto(Module):
	...
	def validate(opt_dict):
		valid = True
		if len(opt_dict.keys()) == len(Module_HTTPnikto.opt.keys()):
			for k,v in opt_dict.items():
				valid = valid and Module_HTTPnikto.opt.get(k,None)(v)
		else:
			valid = False
		return valid
	
	def getName():
		return "Module_HTTPnikto"
	...
	def run(self):
		lst = Module_HTTPnikto.targets(self.opt_dict["target"])
		data = {}
		for ip in lst:
