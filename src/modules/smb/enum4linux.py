...
class Module_SMB_enum4linux(Module):
	...
	def validate(opt_dict):
		valid = True
		if len(opt_dict.keys()) == len(Module_SMB_enum4linux.opt.keys()):
			for k,v in opt_dict.items():
				valid = valid and Module_SMB_enum4linux.opt.get(k,None)(v)
		else:
			valid = False
		return valid
		...
		for ip in lst:
			if not self.flag.is_set():
				...
				if self.mode == "profile": 
					fd = open(Config.PATH+"/db/sessions/"+Config.SESSID+"/profiles/"+self.profile_tag+"/"+ip+"/"+self.profile_port+"/enum4linux","w")
					fd.write(out)
					fd.close()
				data[ip] = out
