...
class Module_Ping3(Module):
	...
	def validate(opt_dict):
		valid = True
		if len(opt_dict.keys()) == len(Module_Ping3.opt.keys()):
			for k,v in opt_dict.items():
				valid = valid and Module_Ping3.opt.get(k,None)(v)
		else:
			valid = False
		return valid
	...
	def run(self):
		lst = Module_Ping3.targets(self.opt_dict["target"])
		time.sleep(20)
		data = {}
		for ip in lst:
