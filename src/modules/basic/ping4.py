...
class Module_Ping4(Module):
	...
	def validate(opt_dict):
		valid = True
		if len(opt_dict.keys()) == len(Module_Ping4.opt.keys()):
			for k,v in opt_dict.items():
				valid = valid and Module_Ping4.opt.get(k,None)(v)
		else:
			valid = False
		return valid
	...
	def run(self):
		lst = Module_Ping4.targets(self.opt_dict["target"])
		time.sleep(15)
		data = {}
		for ip in lst:
